...
- `human_authorized_in_review_removal` obrigatorio como `true` para remover item de `In Review` para qualquer coluna que nao seja `Deploy` ou `Done`
- `devops_in_review_removal` obrigatorio como `true` junto da autorizacao humana
- `in_review_removal_reason` obrigatorio com o motivo humano documentado
...
Protecao de `In Review`: a coluna significa task **individual** ja em staging aguardando conferencia humana. O runner recusa `In Review` → `Working`/`Ready`/outras colunas operacionais quando a operacao nao declara autorizacao humana explicita e remocao conduzida por DevOps. Transicoes normais permitidas: permanecer em `In Review`, seguir para `Deploy` por aprovacao humana, ou ir para `Done` apos publicacao DevOps (Deploy → master). Nao existe pacote, freeze ou inventario de filhas.
