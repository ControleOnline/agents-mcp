...
Protecao de `In Review`: a coluna significa task individual ja em staging aguardando conferencia humana. O runner recusa `In Review` → `Working`/`Ready`/outras colunas operacionais quando a operacao nao declara autorizacao humana explicita e remocao conduzida por DevOps. Transicoes normais permitidas: permanecer em `In Review`, seguir para `Deploy` por aprovacao humana, ou ir para `Done` apos publicacao DevOps. Nao existe pacote, freeze ou inventario de filhas.
