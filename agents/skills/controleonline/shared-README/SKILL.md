...
- `staging` = RC inventariado promovido pelo `DevOps` (pacote congelado)
- apos `Deploy` do RC: `DevOps` mescla o pacote inventariado → `master`; com quarteto → `Done`, sem quarteto → `Working` para segunda validacao
...
- `DevOps` promove cada RC para **`staging`**, `In Review` → `Deploy` → `master`; com quarteto → `Done`, sem quarteto → `Working` para segunda validacao
