...
- **Antes de criar/congelar a RC**, atualize `package.json.version` e `app.json.expo.version` para a versão estável `X.Y.Z` da RC. Ambos devem ser iguais a `manifest.version` e à versão do nome da branch `rc/X.Y.Z-rc.N`; não grave `-rc.N` nos arquivos de versão do produto.
- Faça a atualização dos arquivos dentro do snapshot pré-freeze e valide a igualdade entre os três valores antes de marcar o manifesto `frozen: true`. Nunca corrija arquivos de versão depois do freeze: qualquer divergência descoberta depois exige descartar a candidata e criar `rc.N+1`.
...
6. Gere o manifesto com essa mesma versão `X.Y.Z`.
