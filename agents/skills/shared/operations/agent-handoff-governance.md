...
5. fluxo tecnico padrao:
   ...
   - documentacao (`tutorial-assistant` / `technical-documenter`): **DevOps no publish (fail-closed)** aplica labels de solicitação ausentes nas filhas de produto antes de dar a publicação por concluída; **Manager na higiene** pode restaurar em **lote** labels de solicitação se `Done`/`closed` ficou sem quarteto; documentadores só marcam `:done` com evidência real; aliases legados `qa:accepted`/`security:accepted` equivalem a `agent:qa:accepted`/`agent:security:accepted` na leitura e devem ser normalizados ao atuar
