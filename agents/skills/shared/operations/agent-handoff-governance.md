...
5. fluxo tecnico padrao:
   ...
   - documentacao (`tutorial-assistant` / `technical-documenter`): **DevOps no publish (fail-closed)** aplica **sempre** labels de solicitação ausentes em **todas** as filhas/tasks (sem isentar governança/hotfix); **não decide** conteúdo nem `:done`; **Manager na higiene** restaura em **lote** solicitações se `Done`/`closed` ficou sem as tags; **só os documentadores** decidem e marcam `:done` (com evidência real); aliases legados `qa:accepted`/`security:accepted` equivalem a `agent:qa:accepted`/`agent:security:accepted` na leitura e devem ser normalizados ao atuar
