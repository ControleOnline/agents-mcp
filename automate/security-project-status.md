...
A leitura operacional de `Security` acontece pela **task/issue** e pela evidencia da entrega (commits na `task-{id}` e merge em `staging`), nao por coluna do projeto e nao por PR do Developer.

PR do Developer **nao existe** no fluxo normal. Ver `skills/shared/github-flow.md`.

## Captura

A automacao de `Security` so pode capturar uma task quando:

- a issue esta aberta
- existe label `agent:security` (ou a fase compartilhada de revisao equivalente)
...
- registre apenas `security:accepted` ou `security:rejected` na task
